...
echo "$INFO $(date "+%d/%b/%Y:%H:%M:%S") CustomFirmware mode with OpenVPN" >> $VPNLOG
...
do
	if [ -e $IPUP ]; then
		sed -ie 's#exit 0#/usr/local/bin/vpnup.sh openvpn\nexit 0#g' $IPUP
...
if [ -e $IPDOWN ]; then
	sed -ie 's#exit 0#/usr/local/bin/vpndown.sh openvpn\nexit 0#g' $IPDOWN
